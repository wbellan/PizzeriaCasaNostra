var menu = [
  {
  "name": "Antipasti", 
  "description": null, 
  "items":[
    {
      "name": "Tagliere <br>(Chopping Board)", 
      "description": "Selection of Italian cheeses and cured meats served with focaccia", 
      "price_1": "15",
      "price_2": "00"
    }, 
    {
      "name": "Prosciutto & Mozzarella", 
      "description": "Prosciutto of Parma & Mozzarella of Bufala served with a side of baby arugula and our focaccia", 
      "price_1" : "12", 
      "price_2" : "95"
    }, 
    {
      "name": "Carpaccio", 
      "description": "Beef carpaccio, flakes of parmesan, baby argula, capers, homemade citronette sauce", 
      "price_1" : "10", 
      "price_2" : "00"
    }, 
    {
      "name": "Caponata", 
      "description": "Eggplant, crushed tomato, celery, onion, Kalamta olives, capers, pine nuts, cooked in sweet and sour", 
      "price_1" : "8", 
      "price_2" : "75"
    }, 
    {
      "name": "Patate Fritte al Tartufo", 
      "description": "Truffle french fries, grated parmesano", 
      "price_1" : "9", 
      "price_2" : "00"
    }, 
    {
      "name": "Foccacia Bread", 
      "description": "Pizza bread, coarse sea salt, garlic, olive oil, rosemary, parmesano", 
      "price_1" : "4", 
      "price_2" : "50"
    }
    ]
  },
  {
  "name": "Insalate", 
  "description": null, 
  "items":[
    {
      "name": "Insalata di Vegetali <br>(Mixed vegetable salad)", 
      "description": "Romaine lettuce, baby spinach, escarole, julienne carrots, cherry tomatoes, fennel, celery, baby arugula, red onion, sweet bell pepper, vinaigrette", 
      "price_1": "7",
      "price_2": "50"
    }, 
    {
      "name": "Insalata Caesar", 
      "description": "Romaine lettuce, focaccia croutons, flakes of parmesan cheese, house made Caesar dressing.<br>Add Chicken $3.50", 
      "price_1" : "7", 
      "price_2" : "95"
    }, 
    {
      "name": "Insalata Fagioli e Tonno <br>(Cannellini bean salad with tuna)", 
      "description": "Cannellini beans, tuna, red onion, celery, parsley, extra virgin olive oil", 
      "price_1" : "7", 
      "price_2" : "95"
    }, 
    {
      "name": "Insalata di Fagiolini <br>(Green bean salad)", 
      "description": "Green beans, cherry tomatoes, potatoes, red onion, vinaigrette", 
      "price_1" : "7", 
      "price_2" : "95"
    }, 
    {
      "name": "Caprese", 
      "description": "Mozzarella di bufala, tomatoes, basil, olive oil", 
      "price_1" : "8", 
      "price_2" : "95"
    },
    {
      "name": "Arugula & Roasted Fennel", 
      "description": "Lemon, extra virgin olive oil, parmesano", 
      "price_1" : "9", 
      "price_2" : "50"
    },
    {
      "name": "Zuppa", 
      "description": "Minestrone soup<br>(Note: Need to figure out how to do cup/bowl pricing 4.50/7.95)", 
      "price_1" : "7", 
      "price_2" : "00"
    }
    ]
  },
  {
  "name": "Gourmet Pizzas", 
  "description": "All pizzas are 12 inches in size and made with house-made tomato sauce from the sunny hills of the region of Campagna in Italy.<br>Any of our pizzas can be made as a calzone in the oven. Gluten Free Pizza Dough* available for an extra $3.00", 
  "items":[
    {
      "name": "Casa Nostra", 
      "description": "Tomato sauce, mozzarella, prosciutto, cherry tomatoes, baby arugula, parmesano, lemon drizzle", 
      "price_1": "14",
      "price_2": "00"
    }, 
    {
      "name": "Marinara", 
      "description": "Tomato sauce, oregano, garlic, extra virgin olive oil", 
      "price_1" : "10", 
      "price_2" : "00"
    }, 
    {
      "name": "Margherita", 
      "description": "Tomato sauce, mozzarella, basil, extra virgin olive oil", 
      "price_1" : "11", 
      "price_2" : "00"
    }, 
    {
      "name": "Pesto", 
      "description": "Tomato sauce, mozzarella, pesto, cherry tomatoes, parmesano", 
      "price_1" : "11", 
      "price_2" : "00"
    }, 
    {
      "name": "Sweet Pepper", 
      "description": "Tomato sauce, mozzarella, sweet pepper, Italian sausage, onion, Kalamata olives", 
      "price_1" : "12", 
      "price_2" : "50"
    },
    {
      "name": "Pepperoni", 
      "description": "Tomato sauce, mozzarella, pepperoni", 
      "price_1": "12",
      "price_2": "00"
    }, 
    {
      "name": "Hawaiana", 
      "description": "Tomato sauce, mozzarella, ham, fresh pineapple", 
      "price_1" : "12", 
      "price_2" : "50"
    },
    {
      "name": "Combination", 
      "description": "Tomato sauce, mozzarella, pepperoni, Italian sausage, sweet pepper, Kalamata olives, onions, mushrooms", 
      "price_1" : "14", 
      "price_2" : "00"
    }, 
    {
      "name": "Carnivora", 
      "description": "Tomato sauce, mozzarella, ham, pepperoni, Italian sausage", 
      "price_1" : "13", 
      "price_2" : "95"
    }, 
    {
      "name": "Vegetarian", 
      "description": "Tomato sauce, mozzarella, sweet pepper, mushrooms, onion, cherry tomatoes, Kalamata olives, artichoke hearts", 
      "price_1" : "12", 
      "price_2" : "95"
    }, 
    {
      "name": "4 Stagioni", 
      "description": "Tomato sauce, mozzarella, artichokes, ham, Kalamata olives, mushrooms", 
      "price_1" : "13", 
      "price_2" : "00"
    },  
    {
      "name": "Funghi", 
      "description": "Tomato sauce, mozzarella, mushrooms, onions, Italian sausage", 
      "price_1" : "13", 
      "price_2" : "00"
    }, 
    {
      "name": "Red Garlic", 
      "description": "Tomato sauce, mozzarella, Kalamata olives, basil, grated garlic, cherry tomatoes, olive oil", 
      "price_1" : "13", 
      "price_2" : "00"
    },
    {
      "name": "Lemon", 
      "description": "Tomato sauce, mozzarella, ham, Robiola cheese (soft cheese), mushrooms, grated lemon zest", 
      "price_1" : "13", 
      "price_2" : "50"
    }
    ]
  },
  {
  "name": "White Pizzas", 
  "description": "All pizzas are 12 inches in size and white pizzas are made without tomato sauce.<br>Any of our pizzas can be made as a calzone in the oven. Gluten Free Pizza Dough* available for an extra $3.00", 
  "items":[
    {
      "name": "Bianca Neve", 
      "description": "Mozzarella cheese", 
      "price_1": "10",
      "price_2": "00"
    },
    {
      "name": "4 Formaggi", 
      "description": "Mozzarella, gorgonzola, swiss cheese, parmesano, black pepper", 
      "price_1": "13",
      "price_2": "00"
    }, 
    {
      "name": "Tuna", 
      "description": "Mozzarella, tuna, carmelized onion, Kalmata olives, pine nuts", 
      "price_1" : "12", 
      "price_2" : "50"
    }, 
    {
      "name": "Carbonara", 
      "description": "Mozzarella, bacon, scrambled egg, parmesano, parsley, black pepper", 
      "price_1" : "12", 
      "price_2" : "50"
    }
    ]
  },
  {
  "name": "Create Your Own Pizza", 
  "description": "Choose from the bases options below and add on toppings", 
  "items":[
    {
      "name": "Focaccia Bread", 
      "description": null, 
      "price_1": "8",
      "price_2": "50"
    }, 
    {
      "name": "Marinara (tomato sauce)", 
      "description": null, 
      "price_1" : "10", 
      "price_2" : "00"
    }, 
    {
      "name": "Biancaneve (mozzarella cheese)", 
      "description": null, 
      "price_1" : "10", 
      "price_2" : "00"
    }, 
    {
      "name": "Alfredo", 
      "description": null, 
      "price_1" : "11", 
      "price_2" : "50"
    }
    ]
  },
  {
  "name": "Additional Toppings", 
  "description": "Additional toppings can be added on to any pizza for an extra charge", 
  "items":[
    {
      "name": "Add $1.50", 
      "description": "Cherry Tomatoes<br>Mushrooms<br>Onions<br>Kalamata Olives<br>Capers<br>Basil<br>Garlic<br>Pesto<br>Bell Peppers", 
      "price_1": null,
      "price_2": null
    }, 
    {
      "name": "Add $2.50", 
      "description": "Baby Arugula<br>Anchovies<br>Ham<br>Eggplant<br>Italian Sausage<br>Fresh Mozzarella<br>Pepperoni<br>Ricotta Cheese<br>Bacon<br>Alfredo Sauce<br>Gorgonzola<br>Swiss Cheese<br>Ribiola Cheese (soft cheese)<br>Parmagiano-Reggiano<br>Roasted Fennel<br>Fresh Pineapple", 
      "price_1" : null, 
      "price_2" : null
    }, 
    {
      "name": "Add $4.00", 
      "description": "Prosciutto de Parma<br>Spicy Sopressata<br>Chicken<br>Mozzarella di Bufala", 
      "price_1" : null, 
      "price_2" : null
    }
    ]
  },
  {
  "name": "Pasta", 
  "description": "You can choose between spaghetti or penne", 
  "items":[
    {
      "name": "Bolognese", 
      "description": "Ragu bolognese", 
      "price_1": "12",
      "price_2": "50"
    }, 
    {
      "name": "Puttanesca", 
      "description": "Tomato sauce, Kalamata olives, anchovies, capers", 
      "price_1" : "12", 
      "price_2" : "00"
    }, 
    {
      "name": "Amatriciana", 
      "description": "Tomato sauce, onion, pancetta, parmesano", 
      "price_1" : "12", 
      "price_2" : "50"
    },
    {
      "name": "Frutti di Mare", 
      "description": "Seafood (*NOTE: should specify what seafood?), white wine, cherry tomatoes", 
      "price_1": "14",
      "price_2": "95"
    }, 
    {
      "name": "Lasagna Bolognese", 
      "description": "Ragu Bolognese, bechamele, parmesano", 
      "price_1" : "13", 
      "price_2" : "95"
    }, 
    {
      "name": "Quattro Formaggi", 
      "description": "Parmesano, swiss cheese, gorgonzola (blue cheese), taleggio, white pepper", 
      "price_1" : "13", 
      "price_2" : "00"
    },
    {
      "name": "Carbonara", 
      "description": "Pancetta, whole egg, parmesano, black pepper, parsley", 
      "price_1": "13",
      "price_2": "00"
    }, 
    {
      "name": "Acciughe", 
      "description": "Anchovies, garlic, bread crumbs, parsley", 
      "price_1" : "11", 
      "price_2" : "00"
    }, 
    {
      "name": "Alfredo", 
      "description": "Heavy cream, butter, parmesano<br>Add chicken $3.50", 
      "price_1" : "12", 
      "price_2" : "00"
    },
    {
      "name": "Aglio e Olio", 
      "description": "Garlic, extra virgin olive oil, parsley", 
      "price_1" : "11", 
      "price_2" : "00"
    },
    {
      "name": "Eggplant Parmigiano", 
      "description": "Marinara, fried eggplant, hard boiled egg, ham, mozzarella and parmesano", 
      "price_1" : "13", 
      "price_2" : "00"
    }
    ]
  },
  {
  "name": "Desserts", 
  "description": "Please note that some sweets may contain or have traces of nuts and nut oils or may have been made alongside other products containing nuts. Pizzeria Casa Nostra cannot be held responsible for any issues resulting from food allergies.", 
  "items":[
    {
      "name": "Panna Cotta", 
      "description": "Sweetened cream dessert topped with a strawberry topping", 
      "price_1": "6",
      "price_2": "00"
    },
    {
      "name": "Salame di Cioccolato", 
      "description": "Chocolate salami with roasted almonds", 
      "price_1": "6",
      "price_2": "00"
    },
    {
      "name": "Tiramisu", 
      "description": "Coffee flavored Italian dessert", 
      "price_1": "6",
      "price_2": "95"
    },
    {
      "name": "Cannolo di Ricotta", 
      "description": "Tube shaped pastry dough filled with sweet cream containing ricotta", 
      "price_1": "6",
      "price_2": "00"
    },
    {
      "name": "Slice of Nutella Sweet Pizza", 
      "description": "Topped with chopped roasted almonds", 
      "price_1": "6",
      "price_2": "00"
    },
    {
      "name": "Apple", 
      "description": "Slices of apple, custard, carmelized sugar", 
      "price_1": "6",
      "price_2": "00"
    },
    ]
  },
  {
  "name": "Kid's Menu", 
  "description": "(You can choose between spaghetti or penne)", 
  "items":[
    {
      "name": "Marinara", 
      "description": "Meat sauce add $1.00", 
      "price_1": "7",
      "price_2": "00"
    },
    {
      "name": "Alfredo", 
      "description": "Meat sauce add $1.00", 
      "price_1": "7",
      "price_2": "00"
    },
    {
      "name": "Chicken Nuggests and Fries", 
      "description": null, 
      "price_1": "7",
      "price_2": "00"
    },
    ]
  },
]