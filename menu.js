var menu = [{
    "name": "Antipasti",
    "description": "Some items may be served raw or uncooked. Consuming raw or undercooked meats, poultry, seafood, shellfish, or eggs may increase your risk of foodborne illness, especially if you have certain medical conditions.",
    "items": [{
        "name": "Tagliere <br>(Chopping Board)",
        "description": "Selection of Italian cheeses and cured meats served with our focaccia.",
        "price_1": "15",
        "price_2": "00"
      },
      {
        "name": "Prosciutto & Mozzarella",
        "description": "Prosciutto di Parma & Mozzarella served with a side of baby arugula and our focaccia. <br>(with Mozzarella di Bufala add $3.00)",
        "price_1": "12",
        "price_2": "95"
      },
      {
        "name": "Carpaccio",
        "description": "Beef carpaccio, flakes of Parmigiano Reggiano, baby arugula, capers, homemade citronette sauce.",
        "price_1": "10",
        "price_2": "00"
      },
      {
        "name": "Caponata",
        "description": "Eggplant, bell pepper, crushed tomato, celery, onion, green olives, capers, pine nuts, cooked in sweet and sour.",
        "price_1": "8",
        "price_2": "75"
      },
      {
        "name": "Patate Fritte al Tartufo",
        "description": "French fries, truffle oil, grated Parmigiano Reggiano.",
        "price_1": "9",
        "price_2": "00"
      },
      {
        "name": "Foccacia Bread",
        "description": "Coarse sea salt, garlic, extra virgin olive oil, rosemary, grated Parmigiano Reggiano.",
        "price_1": "4",
        "price_2": "50"
      }
    ]
  },
  {
    "name": "Insalate & Zuppe",
    "description": null,
    "items": [{
        "name": "Insalata di Vegetali<br>(Mixed vegetable salad)",
        "description": "Mix green salad, julienne carrots, cherry tomatoes, celery, red onion, sweet bell pepper, vinaigrette.",
        "price_1": "7",
        "price_2": "50"
      },
      {
        "name": "Insalata Caesar",
        "description": "Romaine lettuce, flakes of Parmigiano Reggiano, focaccia croutons, house made Caesar dressing. <br>(with Chicken add $3.50)",
        "price_1": "7",
        "price_2": "95"
      },
      {
        "name": "Insalata Fagioli e Tonno <br>(Cannellini bean salad with tuna)",
        "description": "Cannellini beans, tuna in olive oil, red onion, celery, parsley, vinaigrette.",
        "price_1": "7",
        "price_2": "95"
      },
      {
        "name": "Insalata di Fagiolini <br>(Green bean salad)",
        "description": "Green beans, tomatoes, potatoes, red onion, oregano, vinaigrette.",
        "price_1": "7",
        "price_2": "95"
      },
      {
        "name": "Caprese",
        "description": "Mozzarella, tomatoes, basil, extra virgin olive oil. <br>(with Mozzarella di Bufala add $3.00)",
        "price_1": "8",
        "price_2": "95"
      },
      {
        "name": "Arugula & Roasted Fennel",
        "description": "Baby arugula, roasted fennel, flakes of Parmigiano Reggiano, lemon, extra virgin olive oil.",
        "price_1": "9",
        "price_2": "50"
      },
      {
        "name": "Zuppa",
        "description": "Minestrone soup.<br>(Cup: $4.50, Bowl: $7.95)",
        "price_1": "",
        "price_2": ""
      }
    ]
  },
  {
    "name": "Red Pizzas",
    "description": "All pizzas are 12 inches in size and made with house-made tomato sauce from the sunny hills of the region of Campania in Italy.<br>Any of our pizzas can be made as a calzone in the oven. Gluten Free Pizza Dough or 10-inch Cauliflower Pizza Dough is available for an extra $3.00.",
    "items": [{
        "name": "Casa Nostra",
        "description": "Tomato sauce, mozzarella, prosciutto di parma, cherry tomatoes, baby arugula, Parmigiano Reggiano, lemon zest.",
        "price_1": "14",
        "price_2": "00"
      },
      {
        "name": "Marinara",
        "description": "Tomato sauce, oregano, garlic, extra virgin olive oil.",
        "price_1": "10",
        "price_2": "00"
      },
      {
        "name": "Margherita",
        "description": "Tomato sauce, mozzarella, basil, extra virgin olive oil.",
        "price_1": "11",
        "price_2": "00"
      },
      {
        "name": "Pesto",
        "description": "Tomato sauce, mozzarella, pesto, cherry tomatoes, Parmigiano Reggiano.",
        "price_1": "11",
        "price_2": "00"
      },
      {
        "name": "Sweet Pepper",
        "description": "Tomato sauce, mozzarella, bell pepper, Italian sausage, onion, Kalamata olives.",
        "price_1": "12",
        "price_2": "50"
      },
      {
        "name": "Pepperoni",
        "description": "Tomato sauce, mozzarella, pepperoni.",
        "price_1": "12",
        "price_2": "00"
      },
      {
        "name": "Hawaiana",
        "description": "Tomato sauce, mozzarella, ham, fresh pineapple.",
        "price_1": "12",
        "price_2": "50"
      },
      {
        "name": "Combination",
        "description": "Tomato sauce, mozzarella, pepperoni, Italian sausage, bell pepper, Kalamata olives, onions, mushrooms.",
        "price_1": "13",
        "price_2": "50"
      },
      {
        "name": "Carnivora",
        "description": "Tomato sauce, mozzarella, Italian sausage, salami soppressata, ham.",
        "price_1": "13",
        "price_2": "95"
      },
      {
        "name": "Vegetarian",
        "description": "Tomato sauce, mozzarella, bell pepper, mushrooms, onion, cherry tomatoes, Kalamata olives, artichokes.",
        "price_1": "12",
        "price_2": "95"
      },
      {
        "name": "4 Stagioni",
        "description": "Tomato sauce, mozzarella, artichokes, ham, Kalamata olives, mushrooms.",
        "price_1": "13",
        "price_2": "00"
      },
      {
        "name": "Salsiccia e Funghi",
        "description": "Tomato sauce, mozzarella, Italian sausage, mushrooms, onions.",
        "price_1": "13",
        "price_2": "00"
      },
      {
        "name": "Red Garlic",
        "description": "Tomato sauce, mozzarella. <br>(Uncooked: Kalamata olives, basil, grated garlic, cherry tomatoes, extra virgin olive oil)",
        "price_1": "13",
        "price_2": "00"
      },
      {
        "name": "Lemon",
        "description": "Tomato sauce, mozzarella, ham, cream cheese, mushrooms, lemon zest.",
        "price_1": "13",
        "price_2": "00"
      }
    ]
  },
  {
    "name": "White Pizzas",
    "description": "All pizzas are 12 inches in size and white pizzas are made without tomato sauce.<br>Any of our pizzas can be made as a calzone in the oven. Gluten Free Pizza Dough or 10-inch Cauliflower Pizza Dough is available for an extra $3.00.",
    "items": [{
        "name": "Biancaneve",
        "description": "Mozzarella.",
        "price_1": "10",
        "price_2": "00"
      },
      {
        "name": "5 Formaggi",
        "description": "Mozzarella, gorgonzola, swiss cheese, Parmigiano Reggiano, cream cheese, black pepper.",
        "price_1": "13",
        "price_2": "00"
      },
      {
        "name": "Tuna",
        "description": "Mozzarella, tuna, carmelized onion, Kalmata olives, pine nuts.",
        "price_1": "12",
        "price_2": "50"
      },
      {
        "name": "Carbonara",
        "description": "Mozzarella, bacon, scrambled egg, Parmigiano Reggiano, parsley.",
        "price_1": "12",
        "price_2": "50"
      }
    ]
  },
  {
    "name": "Create Your Own Pizza",
    "description": "Choose from the base options below and add on toppings. <br>Any of our pizzas can be made as a calzone in the oven. Gluten Free Pizza Dough or 10-inch Cauliflower Pizza Dough is available for an extra $3.00.",
    "items": [{
        "name": "Marinara",
        "description": "Tomato sauce, oregano, garlic, extra virgin olive oil.",
        "price_1": "10",
        "price_2": "00"
      },
      {
        "name": "Margherita",
        "description": "Tomato sauce, mozzarella, basil, extra virgin olive oil.",
        "price_1": "11",
        "price_2": "00"
      },
      {
        "name": "Biancaneve",
        "description": "Mozzarella.",
        "price_1": "10",
        "price_2": "00"
      },
      {
        "name": "Alfredo",
        "description": "Heavy cream, butter, Parmigiano Reggiano. ",
        "price_1": "11",
        "price_2": "50"
      }
    ]
  },
  {
    "name": "Additional Toppings",
    "description": "Additional toppings can be added on to any pizza for an extra charge.",
    "items": [{
        "name": "Add $1.50",
        "description": "Alfredo Sauce<br>Cherry Tomatoes<br>Mushrooms<br>Onions<br>Kalamata Olives<br>Capers<br>Basil<br>Garlic<br>Pesto<br>Bell Peppers<br>Hard-boiled Egg<br>Caramelized Onions<br>Eggplant",
        "price_1": null,
        "price_2": null
      },
      {
        "name": "Add $2.50",
        "description": "Baby Arugula<br>Anchovies<br>Ham<br>Italian Sausage<br>Fresh Mozzarella<br>Pepperoni<br>Bacon<br>Gorgonzola<br>Swiss Cheese<br>Parmigiano Reggiano<br>Roasted Fennel<br>Fresh Pineapple<br>Tuna<br>Artichoke<br>Chicken",
        "price_1": null,
        "price_2": null
      },
      {
        "name": "Add $4.00",
        "description": "Prosciutto di Parma<br>Spicy Sopressata<br>Mozzarella di Bufala",
        "price_1": null,
        "price_2": null
      }
    ]
  },
  {
    "name": "Pastas",
    "description": "You can choose between spaghetti or penne.",
    "items": [{
        "name": "Bolognese",
        "description": "Ragù Bolognese",
        "price_1": "12",
        "price_2": "50"
      },
      {
        "name": "Puttanesca",
        "description": "Tomato sauce, Kalamata olives, anchovies, capers, red pepper flakes.",
        "price_1": "12",
        "price_2": "00"
      },
      {
        "name": "Amatriciana",
        "description": "Tomato sauce, onion, pancetta, Parmigiano Reggiano.",
        "price_1": "12",
        "price_2": "50"
      },
      {
        "name": "Vongole",
        "description": "Clams, white wine, oregano.",
        "price_1": "14",
        "price_2": "95"
      },
      {
        "name": "Lasagna Bolognese",
        "description": "Ragù Bolognese, bechamele, Parmigiano Reggiano.",
        "price_1": "13",
        "price_2": "95"
      },
      {
        "name": "Quattro Formaggi",
        "description": "Parmigiano Reggiano, fontina, gorgonzola, taleggio, ground pepper.",
        "price_1": "13",
        "price_2": "00"
      },
      {
        "name": "Carbonara",
        "description": "Pancetta, whole egg, Parmigiano Reggiano, black pepper, parsley.",
        "price_1": "13",
        "price_2": "00"
      },
      {
        "name": "Acciughe",
        "description": "Anchovies, garlic, bread crumbs, parsley.",
        "price_1": "11",
        "price_2": "00"
      },
      {
        "name": "Alfredo",
        "description": "Heavy cream, butter, Parmigiano Reggiano.<br>(with Chicken add $3.50)",
        "price_1": "13",
        "price_2": "00"
      },
      {
        "name": "Aglio e Olio",
        "description": "Garlic, extra virgin olive oil, parsley.",
        "price_1": "11",
        "price_2": "00"
      },
      {
        "name": "Tonno al Limone",
        "description": "Tuna in olive oil, lemon.",
        "price_1": "13",
        "price_2": "00"
      }
    ]
  },
  {
    "name": "Others",
    "description": "",
    "items": [{
      "name": "Parmigiana",
      "description": "Tomato sauce, fried eggplant, mozzarella, ham, hard-boiled egg, Parmigiano Reggiano, basil.",
      "price_1": "13",
      "price_2": "00"
    }]
  },
  {
    "name": "Kid's Menu",
    "description": "(You can choose between spaghetti or penne)",
    "items": [{
        "name": "Marinara",
        "description": "Ragù Bolognese add $1.00",
        "price_1": "7",
        "price_2": "00"
      },
      {
        "name": "Alfredo",
        "description": "Ragù Bolognese add $1.00",
        "price_1": "7",
        "price_2": "00"
      },
      {
        "name": "Chicken Nuggets and French Fries",
        "description": null,
        "price_1": "7",
        "price_2": "00"
      },
    ]
  },
  {
    "name": "Desserts",
    "description": "Please note that some sweets may contain or have traces of nuts and nut oils or may have been made alongside other products containing nuts. Pizzeria Casa Nostra cannot be held responsible for any issues resulting from food allergies.",
    "items": [{
        "name": "Panna Cotta",
        "description": "Sweetened cream dessert topped with a strawberry topping.",
        "price_1": "7",
        "price_2": "00"
      },
      {
        "name": "Salame di Cioccolato",
        "description": "Made from cocoa, broken biscuits, butter, egg yolks, roasted almonds, liquor vermouth, served with custard sauce.",
        "price_1": "7",
        "price_2": "00"
      },
      {
        "name": "Tiramisù",
        "description": "Ladyfingers dipped in coffee, layered with a whipped mixture of egg yolks, sugar, cream, and mascarpone cheese, flavoured with cocoa.",
        "price_1": "8",
        "price_2": "95"
      },
      {
        "name": "Cannolo di Ricotta",
        "description": "Cannolo tube filled with sweet ricotta.",
        "price_1": "7",
        "price_2": "00"
      },
      {
        "name": "Nutella Sweet Pizza",
        "description": "Slice of pizza crust, nutella, topped with chopped roasted almonds.",
        "price_1": "6",
        "price_2": "00"
      },
      {
        "name": "Apple Sweet Pizza",
        "description": "Slice of pizza crust, apple, custard, carmelized sugar.",
        "price_1": "6",
        "price_2": "00"
      },
    ]
  }
]